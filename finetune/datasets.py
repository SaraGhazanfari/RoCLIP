...
class COCOFlickrDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.image_processor([[image]]).squeeze(0).to(dtype=self.model.model.dtype, device='cuda',
                                                              non_blocking=True)
        ...
        # return image, torch.tensor(input_ids).squeeze(0), torch.tensor(labels).squeeze(0), torch.tensor(
