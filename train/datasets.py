    ...
    def __getitem__(self, idx):
        ...
        if input_ids.shape[1] < max_length:
            pad_token_tensor = torch.tensor(
                [self.model.tokenizer.pad_token_id] * (max_length - input_ids.shape[1] + 1)).unsqueeze(0)
            print(pad_token_tensor.shape)
            attention_mask_tensor = torch.tensor([False] * (max_length - input_ids.shape[1] + 1)).unsqueeze(0)
